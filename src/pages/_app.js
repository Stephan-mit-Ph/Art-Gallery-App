...
export default function App({ Component, pageProps }) {
	...
	const [artPiecesInfo, setArtPiecesInfo] = useLocalStorageState('art-pieces-info', {
		defaultValue: [],
	});
		...
		if (artPiece) {
			setArtPiecesInfo(
				artPiecesInfo.map((pieceInfo) =>
					pieceInfo.slug === slug ? { slug, isFavorite: !pieceInfo.isFavorite } : pieceInfo
				)
			);
		} else {
	...
	function addComment(slug, newComment) {
		...
		if (artPiece) {
			...
			setArtPiecesInfo([...artPiecesInfo, { slug, isFavorite: false, comments: [newComment] }]);
		}
	}
